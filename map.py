...
from create_layers import create_geojson_done, create_geojson_features
from save import add_info
from utils import load_and_clean_gpx
...
def start_map():
    ...
    col3.metric("Progression totale", f"{progress:.1%}")
    st.progress(progress)

    # add the map
    st.subheader("Carte de la progression")

    if segments:
        map_center = segments[0][0]
    else:
